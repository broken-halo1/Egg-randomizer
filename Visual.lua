...
pcall(function()
	StarterGui:SetCore("SendNotification", {
		Title = "ðŸ“¢ Silentzhub Notice",
		Text = "be aware with scripts they can STEAL stuffs from you",
		Duration = 10
	})
end)
...
local toggleSpawner = createToggleButton("ðŸ¾ Pet Spawner", Color3.fromRGB(0, 170, 127), 5)
local toggleDupe = createToggleButton("ðŸŒ€ Pet Dupe", Color3.fromRGB(65, 130, 65), 40)
local showListBtn = createToggleButton("ðŸ“‹ Show Pet List", Color3.fromRGB(100, 100, 180), 75)
...
dupeBtn.MouseButton1Click:Connect(function()
	...
	if validPets[petName] then
		...
		StarterGui:SetCore("SendNotification", {
			Title = "Grow A Garden",
			Text = "Bypassing... your pet is duping âœ…",
			Duration = 4
		})
	else
		status.Text = "Pet not found / unsupported"
...
local function createPetList()
	...
	title.Text = "ðŸ“‹ Supported Pets for Duping"
	title.TextColor3 = Color3.new(1, 1, 1)
	title.BackgroundTransparency = 1
	title.Font = Enum.Font.GothamBold
	title.TextSize = 16
...
ðŸŽ Join our Discord to win 20x Raccoon!
]]
	...
	copyBtn.Text = "ðŸ“Ž Copy Discord Link"
	...
	copyBtn.MouseButton1Click:Connect(function()
		...
		if success then
			StarterGui:SetCore("SendNotification", {
				...
				Text = "Invite link copied to clipboard âœ…",
				Duration = 3
			})
		else
			StarterGui:SetCore("SendNotification", {
				Title = "Failed",
				Text = "Clipboard not supported âš ï¸",
				Duration = 3
			})
		end
	end)
